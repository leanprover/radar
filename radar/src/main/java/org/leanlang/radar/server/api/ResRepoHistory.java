        ...
        return repo.db().readTransactionResult(ctx -> {
            ...
            List<JsonEntry> entries = ctx
                    ...
                    .map(row -> {
                        ...
                        JsonCommit.Ident author =
                                new JsonCommit.Ident(authorName, authorEmail, authorTime, authorOffset);
                        JsonCommit.Ident committer =
                                new JsonCommit.Ident(committerName, committerEmail, committerTime, committerOffset);
                        JsonCommit commit = new JsonCommit(
                                chash, author, committer, messageTitle, Optional.ofNullable(messageBody));
